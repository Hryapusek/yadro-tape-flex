...
TEST(config, default_config_read)
{
	...
	EXPECT_EQ(inputCharacteristics.read_delay_milliseconds, 1.25);
	EXPECT_EQ(inputCharacteristics.write_delay_milliseconds, 0.0005);
	EXPECT_EQ(inputCharacteristics.move_delay_milliseconds, 0.4);
	EXPECT_EQ(inputCharacteristics.move_one_pos_delay_milliseconds, 0.001);
